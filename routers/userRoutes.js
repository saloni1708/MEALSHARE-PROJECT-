...
router.post("/delete-details/:email/:ngoEmail", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      ...
      const donor = await User.find(); // Assuming User is your Mongoose model for users
...
router.post("/User_singUp", async function (req, res) {
  ...
  try {
    ...
    const message = `Thank you, ${fullName.toUpperCase()}, for connecting with the MealShare organization.`;

    await newUser.save().then((user) => {
      let mailOptions = {
        to: user.email,
        subject: "Welcome To MealShare",
        ...
        attachments: [
          {
            filename: "logoo.jpg",
            path: path.join(__dirname, "..", "public", "img", "logoo.jpg"),
            cid: "logo",
          },
        ],
      };
  ...
  try {
    ...
    const mailOptions = {
      ...
      attachments: [
        {
          filename: "logoo.jpg",
          path: path.join("public", "img", "logoo.jpg"),
          cid: "logo",
        },
      ],
    };
